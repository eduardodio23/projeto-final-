...
# Função para validar data de fabricação
    ...
    while len(estoque) < 10:
        ...
        while True:
            ...
            if not data_fabricacao:
                print("A data de fabricação não pode estar vazia. Tente novamente.")
                continue
            if not validar_data(data_fabricacao):
...
def consultar_estoque(estoque):
    ...
    for item in estoque:
        if item['produto'] == produto or str(item['codigo']) == produto:
            ...
            print(f"Data Fabricação: {item['data_fabricacao']}")  
            print(f"Valor de Compra: R$ {item['valor_compra']:.2f}")
...
def relatorio_custos(estoque):
    print("\n--- RELATÓRIO DE CUSTOS ---")

    custo_total = sum(item['quantidade'] * item['valor_compra'] for item in estoque)

    custo_semanal = custo_total / 4
    custo_mensal = custo_total
    custo_anual = custo_total * 12

    print(f"\nCusto total atual do estoque: R$ {custo_total:.2f}")
    print(f"Custo semanal médio: R$ {custo_semanal:.2f}")
    print(f"Custo mensal médio: R$ {custo_mensal:.2f}")
